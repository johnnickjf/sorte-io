...
class NumberService:
    ...
    def create_number(self, number: Number) -> Number:
        if self.repository.select(number.id):
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Number already exists")
        if self.repository.select_number_lottery(number.number, number.lottery):
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Number already exists")
